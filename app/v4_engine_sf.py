...
與 7-11 版 (v4_engine.py) 的唯一差異：
  - 上半部撕線區留白（不從原 PDF 擷取圖片）
  - 下半部完全相同：訂單資訊區 / 12 格商品 / 金額表
  - 所有座標常數、繪製邏輯、字型設定皆與 7-11 版一致

維護注意：除 generate_page() 外，其他函式不要與 7-11 版分歧。
若 7-11 版下半部排版有更新，請同步修改本檔的 generate_page()。
"""
...
def draw_order_info(c, F, FB, order, top_y, zx, zw):
    ...
    if buyer and recipient and buyer != recipient:
        txt(c, f"訂購人:{buyer}　　收貨人:{recipient}　　電話:{phone}",
            lx, sep - 17, FB, 11)
    else:
        name_only = recipient or buyer
        txt(c, f"收件人：{name_only}　　電話：{phone}", lx, sep - 17, FB, 11)
    info_bot = sep - 17
    ...
    rem_y    = info_bot - 15   # 框頂部（reportlab y）
...
# ─────────────────────────────────────────
# 2.2.4 右半品項明細框（V4.2 新增，與 7-11 版一致）
#   - 靠右、左界避開撕線區位置(TEAR_X1 右側)，與 7-11 版同位以求版面一致
#   - 由下往上堆疊：第 1 項貼金額表上方
#   - 一品項一框，上半=商品名稱(換行+限高截斷)、
#     下半=分類代碼明細(與左邊商品表同套:痘乳×1 / 綠×2 橘×2…，可換行)
# ─────────────────────────────────────────
...
# ─────────────────────────────────────────
# 主繪製（順豐版：跳過撕線區擷取，上半部留白）
# ─────────────────────────────────────────
def generate_page(pdf_path, page_index, order, output_path):
    F, FB = setup_font()
    # 順豐版：不擷取上半部圖片，留白
    # （pdf_path / page_index 仍保留參數，與 7-11 版簽名一致，方便 main.py 共用呼叫）
...
if __name__ == "__main__":
    ...
    # 順豐版測試：pdf_path / page_index 不會被讀取（已跳過擷取），給任意值即可
    pdf_path = "/mnt/user-data/uploads/V4SF.pdf"

    generate_page(pdf_path, 0,  ORDER_NO_REMARK,  "/mnt/user-data/outputs/v4_sf_no_remark.pdf")
    generate_page(pdf_path, 0, ORDER_WITH_REMARK, "/mnt/user-data/outputs/v4_sf_with_remark.pdf")
